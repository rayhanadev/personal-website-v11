...
const TrustedPhoneNumbers = z
  .array(
    z.object({
      id: z.number(),
      number: z.string().nullable().optional(),
      numberWithDialCode: z.string().nullable().optional(),
    }),
  )
  .optional();

const TwoFactorChallenge = z.object({
  authType: z.string().nullable().optional(),
  phoneNumberVerification: z
    .object({
...
export type RawFindMyDevice = z.infer<typeof RawFindMyDevice>;
export type RawFindMyLocation = z.infer<typeof RawFindMyLocation>;
export type FindMyClientResponse = z.infer<typeof FindMyClientResponse>;
...
export type ICloudWebSession = {
  ...
  /** ISO date the current trust token was minted, for expiry warnings. */
  mintedAt?: string;
};

export type FindMyConfig = {
  appleId: string;
  deviceName?: string;
  password: string;
...
export type TrustedPhoneNumber = {
  id: number;
  numberWithDialCode: string | null;
};
...
export class AppleHttpError extends Error {
  constructor(
    message: string,
    readonly status: number,
  ) {
    super(message);
    this.name = "AppleHttpError";
  }
}

/**
 * Apple answers a request carrying a spent session token with 421/450/500-ish
 * re-auth codes rather than a plain 401. A stored session token is always
 * going to age out, so these mean "sign in again", not "give up".
 */
const STALE_SESSION_STATUSES = new Set([401, 421, 450]);

export class ICloudWebClient {
  ...
  async fetchFindMyDevices(): Promise<FindMyClientResponse> {
    try {
      return await this.requestFindMyDevices();
    } catch (error) {
      const stale =
        error instanceof AppleHttpError &&
        STALE_SESSION_STATUSES.has(error.status) &&
        (this.sessionData.session_token != null || this.cookies.size > 0);

      if (!stale) {
        throw error;
      }

      // Stale X-APPLE-WEBAUTH-* cookies are what Apple actually rejects here,
      // and a fresh sign-in does not reliably replace every one of them, so
      // clear the jar as well as the token. The trust token keeps SRP 2FA-free.
      this.cookies.clear();
      this.sessionData.session_token = undefined;
      return this.requestFindMyDevices();
    }
  }

  private async requestFindMyDevices(): Promise<FindMyClientResponse> {
  ...
  private async authenticate(): Promise<AccountLoginResponse> {
    if (this.sessionData.session_token) {
      try {
        return await this.accountLogin();
      } catch {
        this.sessionData.session_token = undefined;
      }
    }
  ...
  async signin(): Promise<SigninResult> {
    ...
    if (complete.status === 409) {
      const challenge = parseTwoFactorChallenge(complete.json);

      if (challenge.trustedPhoneNumbers.length === 0) {
        try {
          const state = await this.fetchAuthState();
          return {
            status: "two-factor-required",
            authType: challenge.authType ?? state.authType,
            trustedPhoneNumbers: state.trustedPhoneNumbers,
          };
        } catch {
          // Fall through to whatever the 409 body carried.
        }
      }

      return { status: "two-factor-required", ...challenge };
    }
  ...
  async fetchAuthState(): Promise<{ raw: unknown } & ReturnType<typeof parseTwoFactorChallenge>> {
    ...
    return { raw: response.json, ...parseTwoFactorChallenge(response.json) };
  }

  async sendTrustedDeviceCode(): Promise<void> {
  ...
  async sendPhoneCode(phoneId: number): Promise<void> {
    await this.requestJson(
      "PUT",
      `${AUTH_ENDPOINT}/verify/phone`,
      { mode: "sms", phoneNumber: { id: phoneId } },
      undefined,
      this.authHeaders(),
    );
  }

  async verifySecurityCode(code: string, phoneId?: number): Promise<void> {
    const [url, body] =
      phoneId == null
        ? [`${AUTH_ENDPOINT}/verify/trusteddevice/securitycode`, { securityCode: { code } }]
        : [
            `${AUTH_ENDPOINT}/verify/phone/securitycode`,
            { mode: "sms", phoneNumber: { id: phoneId }, securityCode: { code } },
          ];

    await this.requestJson("POST", url, body, undefined, this.authHeaders());
  }
  ...
  async trustSession(): Promise<void> {
    ...
    if (!this.sessionData.trust_token) {
      throw new Error("Apple did not return a trust token when trusting this session.");
    }

    if (!this.sessionData.session_token) {
      throw new Error("Apple did not return a web session token when trusting this session.");
    }
  }

  exportSession(): ICloudWebSession {
    return {
  ...
  async accountLogin(): Promise<AccountLoginResponse> {
    if (!this.sessionData.session_token) {
  ...
  private async requestJson(
    ...
    const json = response.body.trim() ? parseAppleJson(response.body, rawUrl) : undefined;

    if (response.status >= 400 && response.status !== 409) {
      throw new AppleHttpError(
...
function parseTwoFactorChallenge(json: unknown): {
  authType: string | null;
  trustedPhoneNumbers: TrustedPhoneNumber[];
} {
  const parsed = TwoFactorChallenge.safeParse(json);
  if (!parsed.success) {
    return { authType: null, trustedPhoneNumbers: [] };
  }

  const verification = parsed.data.phoneNumberVerification;
  const phones = parsed.data.trustedPhoneNumbers ?? verification?.trustedPhoneNumbers ?? [];

  return {
    authType: parsed.data.authType ?? verification?.authenticationType ?? null,
    trustedPhoneNumbers: phones.map((phone) => ({
      id: phone.id,
      numberWithDialCode: phone.numberWithDialCode ?? phone.number ?? null,
    })),
  };
}
